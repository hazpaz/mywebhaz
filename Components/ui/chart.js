...
var ChartContainer = React.forwardRef(function (_a, ref) {
    ...
    data - chart, { chartId: chartId }, ref = { ref: ref }, className = {}, __assign({}, props) >
    id, { chartId: chartId }, config = { config: config } /  >
    { children: children }
...
return className;
{
    (0, utils_1.cn)("font-medium", labelClassName);
}
 > { value: value } < /div>;
[
    label,
...
"grid gap-1.5" >
    { payload: payload, : .map(function (item, index) {
            ...
            return key = { item: item, : .dataKey };
            className = { cn: function (, indicator) { } } === "dot" && "items-center";
        }) }
    >
        { formatter: formatter } && (item === null || item === void 0 ? void 0 : item.value) !== undefined && item.name ? (formatter(item.value, item.name, item, index, item.payload)) : { itemConfig: itemConfig, : .icon /  >
 }(!hideIndicator && className, { cn: function (, _a) {
        var indicator = _a["h-2.5 w-2.5"];
    } } === "dot", "w-1", indicator === "line", "w-0 border-[1.5px] border-dashed bg-transparent", indicator === "dashed", "my-0.5", nestLabel && indicator === "dashed");
style = {};
{
    "--color-bg";
    indicatorColor,
...
className;
{
    (0, utils_1.cn)("flex flex-1 justify-between leading-none", nestLabel ? "items-end" : "items-center");
}
    >
        className;
"grid gap-1.5" >
...
className = "text-muted-foreground" >
    { itemConfig: itemConfig, label: label } || item.name;
/span>
    < /div>;
{
...
var ChartLegendContent = React.forwardRef(function (_a, ref) {
    ...
    return ref = { ref: ref };
    className = { cn: function (, verticalAlign) { } } === "top" ? "pb-3" : "pt-3",
        className;
});
exports.ChartLegendContent = ChartLegendContent;
    >
        { payload: payload, : .map(function (item) {
                ...
                var itemConfig = getPayloadConfigFromPayload(config, item, key);
                return key = { item: item, : .value };
                className = {}
                    >
                        { itemConfig: itemConfig, icon: icon } && !hideIcon ? /> : className = "h-2 w-2 shrink-0 rounded-[2px]";
                style = {};
                {
